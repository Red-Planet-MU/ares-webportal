...
export default Controller.extend({    
    ...
    /* These callbacks are wired up a little weird.  
       ...
       In the template initialization, we connect these properties to a property in the component:
           {{tds-chargen model=model updateCallback=tdsUpdateCallback ... }}

       Now the component's updateCallback ---binds to---> tdsUpdateCallback, which is null
   
       THEN in the component's didInsertElement method, we UPDATE updateCallback to point to a function.
  
           this.set('updateCallback', function() { return self.onUpdate(); } );
  
       Because of the binding, setting updateCallback in the component ALSO sets tdsUpdateCallback here in the controller
  
       So now tdsUpdateCallback references a function in the component that we can call to build the query data.
    */
    tdsUpdateCallback: null,
    tdsValidateCallback: null,
  ...
  anyGroupMissing: computed('model.char.groups', function () {
    ...
    buildQueryDataForChar: function() {
      ...
      let tds = this.tdsUpdateCallback ? this.tdsUpdateCallback() : null;
    ...
    actions: {
        ...
        save() {
            ...
            .then( (response) => {
                ...
                if (this.tdsValidateCallback) {
                  this.tdsValidateCallback();
                }
                if (response.alerts) {
